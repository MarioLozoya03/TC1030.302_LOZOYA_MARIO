...
ostream &operator <<(ostream &out, const Serie &v){
    // concatenar todos los episodios de la serie
    string acum = "\n"; 
    for(int index=0; index < v.cantidad; index ++){
        acum = acum + to_string(index) + '-' + v.episodios[index].str() + '\n';
    }

    out << v.iD << ',' << v.titulo << ',' << v.duracion << ',' << v.genero << ',' << v.calificacionPromedio << acum;
  return out;
}
